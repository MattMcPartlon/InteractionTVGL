...
NOdists = {'X': 3.06, 'ILE': 3.174216, 'GLN': 3.3115633, 'GLY': 3.2121522, 'GLU': 3.3253117, 'L': 3.2752273,
           'CYS': 3.1867874, 'HIS': 3.2300916, 'G': 3.2121522, 'LYS': 3.2851896, 'PRO': 3.1523004, 'ASN': 3.2947414,
           'A': 3.302093, 'C': 3.1867874, 'E': 3.3253117, 'D': 3.2847655, 'VAL': 3.1252666, 'F': 3.2107856,
           'I': 3.174216, 'H': 3.2300916, 'K': 3.2851896, 'M': 3.2734911, 'THR': 3.1898365, 'N': 3.2947414,
           'Q': 3.3115633, 'P': 3.1523004, 'S': 3.2022767, 'ASP': 3.2847655, 'T': 3.1898365, 'W': 3.1940844,
           'V': 3.1252666, 'Y': 3.1847978, 'TRP': 3.1940844, 'SER': 3.2022767, 'PHE': 3.2107856, 'ALA': 3.302093,
...
CaCgdists = {'X': 2.2, 'ILE': 2.529928, 'GLN': 2.5625532, 'GLY': 0.0, 'GLU': 2.562029, 'L': 2.6051538, 'CYS': 2.8013322,
             'HIS': 2.5359476, 'G': 0.0, 'LYS': 2.564922, 'PRO': 2.3897746, 'ASN': 2.539332, 'A': 1.5232254,
             'C': 2.8013322, 'E': 2.562029, 'D': 2.5434172, 'VAL': 2.527417, 'F': 2.5461287, 'I': 2.529928,
             'H': 2.5359476, 'K': 2.564922, 'M': 2.559007, 'THR': 2.5252295, 'N': 2.539332, 'Q': 2.5625532,
             'P': 2.3897746, 'S': 2.4277365, 'ASP': 2.5434172, 'T': 2.5252295, 'W': 2.543401, 'V': 2.527417,
             'Y': 2.552031, 'TRP': 2.543401, 'SER': 2.4277365, 'PHE': 2.5461287, 'ALA': 1.5232254, 'MET': 2.559007,
             'R': 2.5622983, 'LEU': 2.6051538, 'ARG': 2.5622983, 'TYR': 2.552031}
...
def StructureParser(pdbfile):
    if pdbfile.endswith('.pdb'):
        ...
        print()
        'ERROR: a protein structure file shall end with either .pdb or .cif'
        exit(1)
    return parser
...
def BuildVirtualCB(residue):
    if residue.get_resname().upper() != 'GLY':
        print()
        'ERROR: you shall not build a virtual Cb atom for non-Glycine residue'
        exit(1)
    ...
    try:
        ...
        print()
        'WARNING: failed to obtain all of N, C and CA atoms in building a virtual Cb atom for residue ', residue.get_full_id()
        return None
...
def SaveOneChain2File(pdbfile, chainName, savefile=None):
    ...
    if not existing:
        print()
        'ERROR: the specified chain ', chainName, ' does not exist in pdbfile: ', pdbfile
        return
...
def ExtractCoordinatesByMapping(sequence, seq2pdb_mapping, residueList, atoms=['CB', 'CA', 'N', 'O', 'CG'],
...
def ExtractCoordinatesBySeq(sequence, pdbfile, atoms=['CB', 'CA', 'N', 'O', 'CG'], maxMisMatches=5, minMatchRatio=0.5,
                            bUseAlternativeAtoms=True):
    seq2pdb_mapping, residueList, pdbseq, chain, numMisMatches, numMatches = MapSeq2PDB(sequence, pdbfile,
                                                                                        maxMisMatches=maxMisMatches,
                                                                                        minMatchRatio=minMatchRatio)
    if seq2pdb_mapping is None:
        return None, None, None, None

    atomCoordinates, numInvalidAtoms = ExtractCoordinatesByMapping(sequence, seq2pdb_mapping, residueList, atoms=atoms,
...
def PostProcessDistMatrix(sequence, distMatrix, eps=0.0001):
    ...
    if 'NO' in distMatrix:
        ...
        for AA, i in zip(sequence, list(range(seqLen))):
...
def CalcTwoROriMatrix(coordinates):
    ...
    for i in range(seqLen):
        ...
        if ValidCB(ci):
            cicb = ci['CB']
        elif 'vCB' in ci and (ci['vCB'] is not None):
            cicb = ci['vCB']
        else:
            continue
        ...
        for j in range(seqLen):
            ...
            if ci['CA'] is not None and cj['CA'] is not None:
                ...
                elif 'vCB' in cj and cj['vCB'] is not None:
...
def CalcCaOriMatrix(coordinates):
    ...
    for i in range(seqLen - 1):
        ...
        for j in range(seqLen - 1):
            ...
            for apt, v in orientation.items():
                oriMatrix[apt][i, j] = v
...
def ExtractCoordinatesNDSSPBySeq(sequence, pdbfile, atoms=['CB', 'CA', 'N', 'O', 'CG'], maxMisMatches=5,
                                 minMatchRatio=0.5, bUseAlternativeAtoms=True):
    ...
    coordinates = ExtractCoordinatesByMapping(sequence, seq2pdb_mapping, residueList, atoms=atoms,
                                              bUseAlternativeAtoms=bUseAlternativeAtoms)

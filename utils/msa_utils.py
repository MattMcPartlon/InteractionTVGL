...
def LoadA2MFile(infile):
    ...
    if not all(check):
        print()
        'inconsistent sequence length in a2m file: ', infile
        exit(1)
...
def CalcSeqWeight(sequences):
    ...
    for s1, i, l1 in zip(sequences, list(range(numSeqs)), requiredIDs):
        for s2, j, l2 in zip(sequences, list(range(numSeqs)), requiredIDs):
            if i >= j:
                continue
...
def CCalcPairwiseMatrix(sequences, weight=None, box=None, matrixType='covariance'):
    ...
    CalcPairwiseMatrix.argtypes = [POINTER(c_char_p), POINTER(c_float), c_int, c_int, POINTER(c_int), c_int,
    ...
    resultMatrix = np.ctypeslib.as_array(pairMatrix, shape=(blockSize,)).reshape(
        (bounds[2] - bounds[0], bounds[3] - bounds[1], 441))
